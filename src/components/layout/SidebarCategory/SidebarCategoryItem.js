...
const SidebarCategoryItem = ({ item, alpha, param }) => {
  ...
  const isDropDownItemActive = (item) => {
    const path = item.new ? `new/${item.word.toLowerCase()}` : item.category;
      ...
      <Nav navbar>
        <Dropdown open={dropdownOpen} toggle={toggleDropdown}>
          ...
          <div className={isDropDownActive()} >
            ...
            {!param ? (
              <>
                {items.filter(item => item.hasOwnProperty("new")).length ?
                  ...
                  {item.group &&
                    items.map((item1, key) => (
                      <DropdownItem key={key} className={isDropDownItemActive(item1)}>
                        ...
                        {item1.new ?
                          <Link to={`${basePath}/new/${Store.formatString(item1.word)}`}>
